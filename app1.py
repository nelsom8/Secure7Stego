...
from cripto_utils1 import encriptar_imagem
from decripto_utils1 import decriptar_imagem
...
class Encriptador:
    def __init__(self):
        ...
        self.texto_label = tb.Label(self.janela, text='Texto a ser Criptografado e Descriptografado:', font=("Arial", 14, "bold"))
    ...
    def encriptar(self):
        ...
        if not imagem_saida_path:
            imagem_saida_path = os.path.join(os.getcwd(), "imagens_criptografadas", f"encrypted_{int(time.time())}.png")
            os.makedirs(os.path.dirname(imagem_saida_path), exist_ok=True)
            self.imagem_saida_entry.delete(0, "end")
            self.imagem_saida_entry.insert(0, imagem_saida_path)
